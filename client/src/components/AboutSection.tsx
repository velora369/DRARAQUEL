          ...
          <div className={`space-y-8 order-1 lg:order-2 ${isVisible ? "animate-slide-up" : "opacity-0"}`}>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {highlights.map((item, index) => (
                <div
                  key={index}
                  className={`glass-card p-3 sm:p-4 rounded-2xl flex items-start sm:items-center gap-3 sm:gap-4 hover-lift transition-all duration-300 ${isVisible ? "animate-slide-up" : "opacity-0"}`}
